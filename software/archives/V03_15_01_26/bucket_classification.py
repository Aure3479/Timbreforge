...
# =============================================================================
# BUCKET CLASSIFICATION (VOICE → HAPTICS) — WHAT THIS DOES
# =============================================================================
#
# Goal:
#   Convert a live voice signal into a small number of stable control channels
#   suitable for driving a limited number of vibration actuators.
#
# Key design (two-layer architecture):
#   (1) CANONICAL LAYER (stable, high-resolution):
#         - Use K perceptually spaced filters (default K=32 ERB bands)
#         - Compute canonical energies e (length K)
#           e = W @ P
#           where:
#             P = |rfft(frame)|^2 is the FFT power spectrum
#             W is a triangular filterbank (K x n_bins)
#
#   (2) OUTPUT LAYER (variable, low-resolution):
#         - Decide M output channels (M ≈ number of actuators available)
#         - Merge canonical energies into M channels:
#           b = A @ e
#           where:
#             A is the aggregation matrix (M x K)
#
#   Only A changes when you change actuator count, bucket boundaries, or overlap.
#   W stays the same (so experiments remain comparable).
#
# Main parameters (what / why / typical values):
#   sr (Hz):
#     - Audio sample rate. Must satisfy fmax <= sr/2 (Nyquist).
#     - Use 48000 if you want to analyse up to 12 kHz.
#
#   n_fft:
#     - FFT size. Larger => better frequency resolution, higher latency/CPU.
#     - Typical: 1024–2048 for realtime voice at 48 kHz.
#
#   fmin, fmax (Hz):
#     - Analysis range. Typical voice feedback: 50 Hz to 8–12 kHz.
#     - fmin ~50 covers low F0 and voiced energy.
#     - fmax ~8k captures articulation/noise; up to ~12k can capture “air/halo”
#       IF your mic + SR support it (requires sr >= 24000; we use 48000).
#
#   K:
#     - Canonical band count (front-end). Keep fixed across experiments.
#     - 32 is a common sweet spot: enough detail to re-map later, still light.
#
#   canonical_scale:
#     - How canonical bands are spaced. "erb" recommended for voice.
#     - ERB spacing approximates auditory filter bandwidths (more resolution
#       where hearing/voice cues are dense, less where it matters less).
#
#   M:
#     - Output channel count (usually = number of actuators you can control now).
#
#   bucket_method:
#     - "voice_landmark":
#         Semantic regions aligned to voice cues (F0, F1, F2, presence, noise...).
#         Best when M is small (6–12) and learnability matters.
#     - "erb"/"mel"/"log"/"linear":
#         Baseline strategies for systematic comparisons or non-voice audio.
#
# Overlap:
#   - In GUI experiments you may allow buckets to overlap.
#   - Overlap is implemented by letting a canonical band contribute to multiple
#     buckets (A rows share columns), then normalizing each bucket row.
#
# When to use which mode:
#   NORMAL MODE (spectral buckets):
#     - Direct, predictable, simplest to debug.
#     - Best for early PoC, latency tests, “does it vibrate when I sing?”.
#
#   COLOUR MODE (timbre colours):
#     - Channels are semantic (warmth/presence/shimmer/etc.) defined as weighted
#       sums over canonical ERB bands.
#     - Best for pedagogy: user can learn “what they are doing” (tone quality)
#       rather than only “which frequency band is loud”.
#
# =============================================================================
...
def default_voice_regions(fmin: float, fmax: float) -> List[Region]:
    ...
    # -----------------------
    # Many "voice quality" cues live below ~8 kHz, which is why 8 kHz is a common
    # pragmatic ceiling for speech/singing analysis.
    #
    # However, in *some* singers and in some recording chains, a perceptible
    # "halo" or "air" component can extend above 8 kHz (roughly 8–12 kHz).
    #
    # To keep the mapping future-proof, we include an optional halo region
    # (8000–12000 Hz). It is automatically removed if fmax < 8000.
    regions = [
        ...
        Region("halo_extension",    8000.0, 12000.0, 0.7),
    ]
...
# -------------------------------------------------------------------------
# Aggregation matrix builder (canonical -> buckets)
# -------------------------------------------------------------------------

def build_A_from_bucket_edges(
    ...
    *,
    ...
    """Build the aggregation matrix **A** that merges canonical bands into buckets.

    Parameters
    ----------
    canonical_centers_hz:
        Array of length K giving the center frequency (Hz) of each canonical band.

    bucket_edges_hz:
        List of M (lo, hi) bucket intervals in Hz.

    allow_overlap:
        - False (default): each canonical band is assigned to exactly ONE bucket
          (the first bucket whose interval contains the band center).
        - True: a canonical band can contribute to MULTIPLE buckets if its center
          lies inside multiple bucket intervals. This is useful when the GUI
          allows bucket overlap for experimentation.

    normalize:
        - "mean": divide each bucket row by the number of contributing canonical
          bands (default). This makes bucket energies comparable even if some
          buckets are wider than others.
        - "sum": leave rows un-normalized (energy grows with bucket width).

    Returns
    -------
    A : np.ndarray, shape (M, K)
        Aggregation matrix such that b = A @ e.

    Notes
    -----
    We intentionally use **canonical band centers** for the assignment test.
    This is cheap and stable, and works well for tactile mapping.
    """
    M = len(bucket_edges_hz)
    K = int(len(canonical_centers_hz))
    if M < 1 or K < 1:
        raise ValueError("Need at least 1 bucket and 1 canonical band")

    A = np.zeros((M, K), dtype=np.float32)
    counts = np.zeros(M, dtype=np.int32)

    for k, fc in enumerate(canonical_centers_hz):
        hits = []
        for i, (lo, hi) in enumerate(bucket_edges_hz):
            # last bucket includes the right edge to avoid edge loss
            in_bucket = (fc >= lo and fc < hi) or (i == M - 1 and fc <= hi)
            if in_bucket:
                hits.append(i)
        ...
        if not hits:
            # Numerical safety: clamp out-of-range bands.
            hits = [0] if fc < bucket_edges_hz[0][0] else [M - 1]
    ...
    if normalize == "mean":
        for i in range(M):
            if counts[i] > 0:
                ...
                # Rare: if no canonical centers fall into a bucket, copy nearest
                # non-empty row to avoid dead channels.
...
def update_bucket_edges(
    ...
    *,
    ...
    """Update an existing BucketConfig in-place with new bucket edges.

    Why this function matters
    -------------------------
    In a GUI experiment, you often want to tweak bucket boundaries live
    (move splits, create overlap, merge regions) without rebuilding the
    canonical front-end. This function keeps the canonical layer fixed and
    only rebuilds A.

    Requirements
    ------------
    - len(bucket_edges_hz) must equal cfg.M (we are not changing actuator count).

    Side effects
    ------------
    - cfg.bucket_edges_hz is replaced
    - cfg.A is replaced
    """
    if len(bucket_edges_hz) != cfg.M:
        raise ValueError(
            f"Expected {cfg.M} bucket edges, got {len(bucket_edges_hz)}. "
            "(Changing M requires building a new config.)"
        )
    canonical_centers = cfg.canonical_edges_hz[1:-1]
    cfg.bucket_edges_hz = [(float(lo), float(hi)) for lo, hi in bucket_edges_hz]
    cfg.A = build_A_from_bucket_edges(
        canonical_centers,
        cfg.bucket_edges_hz,
...
def make_bucket_config(
    ...
    #
    # We build A from bucket_edges. By default we keep allow_overlap=False here
    # (each canonical band maps to a single bucket) for predictable baseline
    # behaviour.
    #
    # If you want to allow overlapping buckets (e.g. GUI experimentation),
    # call update_bucket_edges(cfg, ..., allow_overlap=True).
    K_actual = canonical_W.shape[0]
    canonical_centers = canonical_edges_hz[1:-1]  # length K
    A = build_A_from_bucket_edges(
        canonical_centers,
        bucket_edges,
        allow_overlap=False,
        normalize="mean",
...
def compute_canonical_energies(frame: np.ndarray, cfg: BucketConfig) -> np.ndarray:
    """Compute canonical band energies (size K) for a single time-domain frame.

    This is the stable "front-end" representation that sits under all bucket
    and colour strategies.

    Returns
    -------
    e : np.ndarray, shape (K,)
        Canonical energies in each ERB/Mel/log/linear band.
    """
    X = np.fft.rfft(frame, n=cfg.n_fft)
    P = (np.abs(X) ** 2).astype(np.float32)
    return cfg.canonical_W @ P
